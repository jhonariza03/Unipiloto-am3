$(document).ready(function(){
	    ...
	    function Validate(){}; /*Forma de heredar en javascript*/

  		Validate.prototype = {
  			email : function(email){
      			var pattern = new RegExp(/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]+$/);
      			return pattern.test(email); //forma de aplicar la expresion regular
    	},
    	userid : function(uid){
		  ...
		  $('#buttons #info').remove();
		   ...
		   $('.button').bind('click', function(event){
			    ...
			    if(len<1 && data === ""){
			      ...
			      localStorage.setItem('username', data);  /*funcion de html 5*/
			    }
